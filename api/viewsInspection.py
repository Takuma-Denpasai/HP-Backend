...
def inspection(request, id):
  ...
  if checkPermission(request.user, id, [PERMISSION_ADMIN, PERMISSION_INSPECTION]):
    ...
    if request.method == 'GET':
      ...
      band_song = list(BandSongInspectionData.objects.filter(ai=True, inspected=False, deleted=False).values('song__name', 'song__band__name'))
...
def inspect(request, id, category, item_id):
  ...
  if checkPermission(request.user, id, [PERMISSION_ADMIN, PERMISSION_INSPECTION]):
    ...
    if request.method == 'POST':
      ...
      inspect_result = request.POST['status'] == 'true'
      ...
      if category == 'news':
        ...
        if news.exists():
          ...
          news.update(inspected=inspect_result, user=request.user)
        ...
        if organization_permission.exists():
          ...
          organization_permission.update(inspected=inspect_result, user=request.user)
        ...
        if post.exists():
          ...
          post.update(inspected=inspect_result, user=request.user)
        ...
        if shop.exists():
          ...
          shop.update(inspected=inspect_result, user=request.user)
        ...
        if menu.exists():
          ...
          menu.update(inspected=inspect_result, user=request.user)
        ...
        if event.exists():
          ...
          event.update(inspected=inspect_result, user=request.user)
